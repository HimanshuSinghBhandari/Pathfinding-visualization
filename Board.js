class Board {
    constructor(ROWS = 20 , COLS=20){
        //console.log('construcing new board ')
...
async generateRandomMaze() {
  ...
  let intialGrid = this.board[floor(random(0, ROWS))][floor(random(0, COLS))]
  intialGrid.isWall = false
  allFrontiers.push(...this.getFrontier(intialGrid))
    ...
    if(neigbours.length > 0){
        ...
        if (currentFrontier.i !== currentPath.i ){
          ...
          for(let frontier of this.getFrontier(currentFrontier)) {
            ...
            let carve = this.board[(currentFrontier.j + currentPath.j) /2][currentFrontier.i]
            carve.isWall = false;
            currentFrontier.isWall = false
            for(let frontier of this.getFrontier(currentFrontier)) {
              arrAdd(allFrontiers , frontier)
    ...
    for(let dy = -1 ; dy <=1 ; dy ++){
        for(let dx = -1 ; dx <=1 ; dx++){
            ...
            if(newi >=0 && newi < ROWS && newj >= 0 && newj < COLS){
                neigbours.push(this.board[newi][newi])
            }
        }
    }
    return neigbours
